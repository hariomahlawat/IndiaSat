...
def ComputeAccuracy_goldblatt(cropped_images_folder, text_file):
	# specifying integer code of land cover classes
	built_up = 1
	non_built_up = 2
	landcover_classes = ['BU','NBU']
	...
	for landcover in landcover_classes:
		...
		for tif_file in os.listdir(tif_files_path):
			tif_image = np.asarray(Image.open(tif_files_path+'/'+tif_file))
			if non_built_up in np.unique(tif_image, return_counts=True)[0]:
				landcover_predicted_class_count[1] += np.unique(tif_image, return_counts=True)[1][np.where(np.unique(tif_image,return_counts=True)[0]==non_built_up)[0][0]]
			if built_up in np.unique(tif_image, return_counts=True)[0]:
				landcover_predicted_class_count[0] += np.unique(tif_image, return_counts=True)[1][np.where(np.unique(tif_image,return_counts=True)[0]==built_up)[0][0]]
...
def main():
	...
	choice = input(
		...
		Enter 1 - for Goldblatt dataset
		Enter 2 - for our own dataset
		''') 
	if choice == '1':
		chosen_folder = input_folder
		heading = '''
		-------------------------
		Goldblatt Dataset
		-------------------------
	...
	print('''
		Step1 - Cut district/area .tif files(classification) using groundtruth shapefiles.
		''')
	folder_tifffiles = chosen_folder
	...
	cropped_images_folder=chosen_folder + '/2cat'
	...
	if choice=='2':
		...
		ComputeAccuracy_goldblatt(cropped_images_folder+'_cropped',text_file_path)
	else:
		print('Wrong choice!')	
